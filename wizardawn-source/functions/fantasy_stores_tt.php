...
$shopres = mysqli_query( $connection, $shopqry ); /* shopqry. */
	...
	while ($shopary=mysqli_fetch_assoc($shopres)) :
		...
		$shpq = "UPDATE shop_track SET shop=$huts, type='$hutname', didit=$doneit WHERE building=$shopary[building] AND shop!=99";
		mysqli_query( $connection, $shpq ); /* shpq. */

	endwhile;

$shopqry = "SELECT * FROM shop_track WHERE shop>0 AND didit=$doneit ORDER BY building";
$shopres = mysqli_query( $connection, $shopqry ); /* shopqry. */
	...
	<?php	if ($owned != "Caretaker"){echo " [" . number_format(mt_rand(50,3000)) . "gp] ";}
			...
			else if ($shopary[shop] > 16)
			{
				...
				while ($guildy > 0) :
					...
					{
						echo "<hr color='#C0C0C0' size='1'><font size='2'>
							The guild will teach spells at the cost of 1,000gp per spell level to other wizards only.&nbsp; They will also cast <i>Detect Magic</i> on an item for 100gp, <i>Omnipotent Eye</i> on an item for 200gp, or <i>For What It's Worth</i> on an item for 300gp. 
							They also sell an ordinary magic staff for 100 gold, or a deluxe magic staff for 5,000 gold.
						</font><br>";
					}
					else if (($guild_does == 1) && ($guild_class == "rogue"))
					{
						echo "<hr color='#C0C0C0' size='1'><font size='2'>The guild will secretly teach spells at the cost of 2,000gp per spell level to those the wizard guild will not teach.&nbsp; They will also appraise an item for 400gp.</font><br>";
					}
			...
			else if ($shopary[shop] == 15)
			{
				...
				while ($guildy > 0) :
					...
					{
						echo "<hr color='#C0C0C0' size='1'><font size='2'>The church will cast spells if a donation is made to the church. They will cast <i>Healing Feeling</i> for 50gp, <i>Too-Bad Toxin</i> for 100gp, <i>Pygmalion</i> or 600gp, or <i>Born Again</i> for 1,000gp.</font><br>";
					}
